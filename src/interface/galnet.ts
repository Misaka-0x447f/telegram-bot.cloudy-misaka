...
const mapGalnetNews = (news: GalnetArticleNode) => ({
...
export type GalnetNews = ReturnType<typeof mapGalnetNews>
...
export const fetchGalnet = async () => {
  ...
  const res = await got
    ...
    .json<{ data: GalnetArticleNode[] }>()

  return res.data.map(mapGalnetNews)
}

export const fetchGalnetBySlug = async (slug: string) => {
  const base = 'https://cms.zaonce.net/en-GB/jsonapi/node/galnet_article/'
  const res = await got
    .get(base, {
      searchParams: {
